<template>
<div class=''>
    <el-input v-model='condition.customer' placeholder="用户" class="input"></el-input>
    <el-input v-model='condition.type' placeholder="类型" class="input"></el-input>
    ...
    <el-table :data="list" border :height="tableHeight">
        ...
        <el-table-column label="操作">
            <template slot-scope="{row}">
                <el-button type="primary" size="small" @click="addRowOrUpdate(row)">编辑</el-button>
    ...
    <el-dialog title="订单" :visible.sync="dialog.show" center>
            ...
            <div slot="footer" class="dialog-footer">
                <el-button size="small" type="primary" @click="dialog.show = false">取消</el-button>
                <el-button size="small" @click="dialog.form.id?editeRow():addRow()">确定</el-button>
            </div>
        </el-dialog>
</div>
</template>
...
export default {
    ...
    methods:{
        getList(){
            this.axios.post("/free/order/queryOrder",{condition:this.condition,pagger:{current:this.current,size:this.size}}).then(res=>{
                if(res.data.code === 0){
                    this.list = res.data.data.resulte || []
        ...
        addRow(){
            let add = ()=> {
                return this.axios.post("/free/order/createOrder",this.dialog && this.dialog.form)
            }
            this.addAndUpdateCallback(add)
        },
        editeRow(){
            let update = ()=>{
                return this.axios.post("/free/order/updateOrder",this.dialog.form)
            }
